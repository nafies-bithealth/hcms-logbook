...
interface PreceptorLogbookDetailProps {
  employee: Employee;
  logbook: LogbookEntry;
  onBack: () => void;
}

export function PreceptorLogbookDetail({ employee, logbook, onBack }: PreceptorLogbookDetailProps) {
  const [showApproveSheet, setShowApproveSheet] = useState(false);
  const [showRejectSheet, setShowRejectSheet] = useState(false);
  const [rejectionReason, setRejectionReason] = useState("");

  const handleApprove = () => {
    console.log("Approve logbook:", logbook.id);
      ...
      {showApproveSheet && (
        <>
          <div
            ...
            onClick={() => setShowApproveSheet(false)}
          />
          <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[402px] bg-white rounded-tl-[24px] rounded-tr-[24px] shadow-[0px_-4px_8px_0px_rgba(0,0,0,0.15)] z-50 flex flex-col">
            <div className="flex items-center justify-between px-[31px] py-[16px] border-b border-[#e4e7ec]">
              ...
              <button onClick={() => setShowApproveSheet(false)}>
            ...
            <div className="p-[16px]">
              <p className="font-['Inter:Regular',sans-serif] text-[#475467] text-[14px] mb-[24px]">
                Are you sure you want to approve this logbook entry? This action will mark the entry as approved.
              </p>
