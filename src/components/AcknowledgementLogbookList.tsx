...
interface AcknowledgementLogbookListProps {
  employee: Employee;
  onBack: () => void;
  onViewDetails: (logbook: LogbookEntry) => void;
}

export function AcknowledgementLogbookList({ employee, onBack, onViewDetails }: AcknowledgementLogbookListProps) {
  const [statusFilter, setStatusFilter] = useState<"all" | "pending" | "acknowledged" | "rejected">("all");
  const [viewMode, setViewMode] = useState<"card" | "table">("table");

  // Mock data - logbooks submitted for acknowledgement by the employee
  ...
  const acknowledgedCount = logbooks.filter(log => log.status === "acknowledged").length;
